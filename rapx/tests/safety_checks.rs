...
fn test_audit_case1() {
    ...
    assert_eq!(
        output.contains("Lack safety annotations"),
        true
    );
}
...
#[test]
fn test_verify() {
    let output = running_tests_with_arg("others/support/safety_check/slice_from_raw_parts", "-verify");
    assert_eq!(
        output.contains("|RAP|WARN|"),
        true
    );
}